...
while True:
	...
	for encoding in encodings:
		...
		if (len(names) == 1):
			lcd.color = [100,0,0]
			lcd.message = "Hi " + names[0] + " !\n How is Your Day?"
		if (len(names) == 2):
			if ((names[0]==names[1])):
				lcd.color = [100,0,0]
				lcd.clear()
				lcd.message = "Enjoy your food " + names[0] + " !"
			else:
				lcd.color = [100,0,0]
				lcd.clear()
				lcd.message = "Stop " + names[0] +"\nThat's " + names[1] + "'s food!"
				detect = True
		print(names)
